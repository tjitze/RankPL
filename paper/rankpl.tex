...
On line 1 the time point $t$ is initialized and the robot's location $\texttt{x}$, $\texttt{y}$ is initialized to a randomly chosen coordinate 
	using nested ranked choice statements (all equally likely).
...
However, after two more iterations (i.e., if we set $k$ to $4$), the program produces the actual location.\looseness=-1
