...
Reasoning tasks that are supported include causal inference, abduction and and different types of (iterated) belief revision.
...
In this section we present the necessary basics of ranking theory, all of which is due to Spohn~\cite{DBLP:books/daglib/0035277}.
The definition of a \emph{ranking function} presupposes a finite set $\Omega$ of possibilities
	and a boolean algebra $\mathcal{A}$ over the subsets of $\Omega$, called \emph{propositions}.

\begin{definition}
A ranking function is a function $\kappa: \Omega \rightarrow \mathbb{N} \cup \{\infty\}$ that associates events with \emph{ranks}.
$\kappa$ is extended to a function over propositions by defining $\kappa(\emptyset) = \infty$ and $\kappa(A) = min( \{ \kappa(w) \mid w \in A \} )$ for each $A \in \mathcal{A} \setminus \emptyset$.
...
Let start with a brief overview of the features of RankPL.
...
Here, the observation rules out the possibility $\texttt{y} = 1$,
...
$\textit{s}$		& $:=$ 	& $\{ s \} \mid s_0; s_1 \mid x := e \mid \textbf{if }b\textbf{ then } s_1\textbf{ else }s_2 \mid $\\
			& 		& $\textbf{while } b \textbf{ do } s \mid s_1 \Rank{e} s_2 \mid \textbf{observe }b \mid \textbf{skip}$\\
\end{tabular}
\end{center}
Some syntactical shortcuts: 
Brackets and parentheses will be omitted unless this leads to ambiguity.
We define $\wedge$ in terms of $\vee$ and $\neg$.
We write $\textbf{if }b\textbf{ then } s$ instead of $\textbf{if }b\textbf{ then } s$ $\textbf{else skip}$,
...
Thus, the semantics will be defined by a partial function
	$$D: S \rightarrow (K^{*} \nrightarrow K^{*}).$$
...
We are now ready to define the semantics of statements, which is captured by seven rules, numbered \textbf{D1} to \textbf{D7}.
...
To execute the $\textbf{if }b\textbf{ then }s_1\textbf{ else }s_2$ statement we first 
...
\end{align}
	The semantics of the observe statement $\textbf{observe }b$ corresponds to conditioning on the valuations satisfying $b$, unless either the rank of this set is $\infty$ or the prior ranking equals $\kappa_{\infty}$ (i.e., represents failure).
\begin{align}
...
\end{align}
	We define the semantics of $\textbf{while }b\textbf{ do }s$
		as the iterative execution of $\textbf{if }b\textbf{ then }s$ $\textbf{else skip}$
...
\begin{equation*}
f(b, s)(\kappa) = \dn{\textbf{if }b\textbf{ then }s\textbf{ else skip}}(\kappa).
\end{equation*}
...
The \emph{circuit diagnosis problem} is about explaining observed incorrect behavior by finding minimal sets of gates that, if faulty, cause this behavior.\footnote{See Halpern~\cite{DBLP:books/daglib/0014219} for a similar treatment of this example.}

\begin{figure}
\centering
\begin{tikzpicture}[circuit logic US, minimum height=6mm]
...
%In what follows we write $\kappa_{b \rightarrow x}$ as shorthand for $\kappa_{\beta_\kappa(b) \rightarrow x}$.
Instead of introducing a special statement representing J-conditioning, 
	we show that we can already express it in RankPL.
Below we write $\kappa_{b \rightarrow x}$ as shorthand for $\kappa_{\beta_\kappa(b) \rightarrow x}$.
...
We then have
$\kappa_{b \rightarrow x} = S[\textbf{observe }b \Rank{x} \textbf{observe }\neg b](\kappa).$ % removed: inline
\end{theorem}

Next, we consider \emph{L-conditioning}~\cite{goldszmidt1996qualitative} (also called \emph{evidence-oriented} conditioning~\cite{DBLP:books/daglib/0035277}).
...
Again, this operation satisfies the DP postulates of iterated belief revision.\looseness=-1
...
The effect of L-conditioning by $A$ with strength $x$ is that the the rank of $A$ improves by $x$ units with respect to the rank of $\neg A$.
...
L-conditioning satisfies two intuitively desirable properties not satisfied by J-conditioning, making it suitable to model noisy observation:
	it is \textit{reversible} ($(\kappa_{A \uparrow x})_{\overline{A} \uparrow x} = \kappa$) and 
	\textit{commutative} ($(\kappa_{A \uparrow x})_{B \uparrow x} = (\kappa_{B \uparrow x})_{A \uparrow x}$)~\cite{DBLP:books/daglib/0035277}.
Again, we will show how to express it in RankPL.
Like before, we will write $\kappa_{b \uparrow x}$ as shorthand for $\kappa_{\beta_\kappa(b) \uparrow x}$.
...
We then have:
	$$\kappa_{b \uparrow x} = S\begin{bmatrix}\textbf{if rank}(b) \leq x\textbf{ then} \\ \textbf{observe }b \Rank{x - \textbf{rank}(b) + \textbf{rank}(\neg b)} \textbf{observe }\neg b \\ \textbf{else} \\ 
	\textbf{observe }\neg b \Rank{\textbf{rank}(b) - x} \textbf{observe }b\end{bmatrix}(\kappa)$$
\end{theorem}
...
The grids in figure~\ref{fig:localization} depict the map, 
	where a gray cell represents a wall and all other cells are empty (ignoring, for now, the red cells and dots).
...
On lines 7-8 the north sensor reading is processed
	by counting the empty cells between the robot and nearest wall,
