...
public class MergingIteratorFixedTest extends RPLBaseTest {

	public void testEqualInput1() throws RPLException {
		for (int i = 0; i < 5; i++) {
			MergingIteratorFixed s = new MergingIteratorFixed(getTestIterator(), getTestIterator(), i);
		...
		for (int i = 0; i < 12; i++) {
			for (int as = 1; as < 6; as++) {
				for (int bs = 1; bs < 6; bs++) {
					int count = 0;
					MergingIteratorFixed s = new MergingIteratorFixed(getIterator("a", as), getIterator("b", bs), i);
					while (s.next()) {
						count++;
	...
	public void testWithEmptyA1() throws RPLException {
		for (int i = 0; i < 10; i++) {
			for (int bs = 1; bs < 6; bs++) {
				MergingIteratorFixed s = new MergingIteratorFixed(new AbsurdIterator<VarStore>(), getIterator("b", bs), i);
				for (int j = 0; j < bs; j++) {
					assert(s.next());
	...
	public void testWithEmptyB1() throws RPLException {
		for (int i = 0; i < 10; i++) {
			for (int as = 1; as < 6; as++) {
				MergingIteratorFixed s = new MergingIteratorFixed(getIterator("a", as), new AbsurdIterator<VarStore>(), i);
				for (int j = 0; j < as; j++) {
					assert(s.next());
	...
	public void testNormalization() throws RPLException {
		for (int i = 0; i < 10; i++) {
			for (int r = 0; r < 10; r++) {
				MergingIteratorFixed s = new MergingIteratorFixed(new AbsurdIterator<VarStore>(), getIterator("a", i), r);
				for (int j = 0; j < i; j++) {
					assert(s.next());
					assertEquals((int)s.getItem().getValue("a", Type.INT), j);
